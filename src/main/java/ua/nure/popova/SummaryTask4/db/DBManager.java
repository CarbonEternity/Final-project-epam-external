...
//    }
     ...
     * @throws DBException
     */
    public Entity findUser(long id) throws DBException {
        Entity entity = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        Connection con = null;
        try {
            con = getConnection();
            pstmt = con.prepareStatement(SQL_FIND_ENROLLEES_BY_ID);
            pstmt.setLong(1, id);
            rs = pstmt.executeQuery();
            if (rs.next()) {
                entity = extractUser(rs);
            }
            con.commit();
        } catch (SQLException ex) {
            rollback(con);
     ...
     * @throws DBException
     */
    public void updateEnrollee(Enrollee user) throws DBException {
        Connection con = null;
        try {
            con = getConnection();
            updateEnrollee(con, user);
            con.commit();
        } catch (SQLException ex) {
            rollback(con);
     ...
     * @throws SQLException
     */
    private void updateEnrollee(Connection con, Enrollee user) throws SQLException {
        PreparedStatement pstmt = null;
        try {
            pstmt = con.prepareStatement(SQL_UPDATE_ENROLLEE);
            int k = 1;
